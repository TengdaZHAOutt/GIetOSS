...
plt.figure()
plt.scatter(donn1[0,:],donn1[1,:])
for i in range(N) :
     plt.text(donn1[0,i],donn1[1,i],nombre[i],va="bottom",ha="center",fontsize=8)
plt.title('Représenter les points/partie C')
plt.show()
...
coord=np.matmul(np.transpose(donn1),np.transpose(vpm))
